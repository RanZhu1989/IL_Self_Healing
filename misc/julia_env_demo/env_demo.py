...
class SelfHealing_env(gym.Env):
    ...
    def __init__(self, 
        ...
        # ---------读取系统数据----------------------
        self.system_data = System_Data(file_name=data_file)
    ...
    def step(self, action:any) -> (dict, float, bool, dict):
        ...
        jl.set_StepModel(X_rec0_input=self._x_load, X_tieline_input=x_tieline_input, Q_svc_input=q_svc_input, vvo=self.vvo)
...
env = SelfHealing_env(solver="cplex",data_file="Case_33BW_Data.xlsx",solver_display=False, min_disturbance=2, max_disturbance=5, vvo=False)
# obs,info = env.reset(disturbance=[6,11,29,32])
