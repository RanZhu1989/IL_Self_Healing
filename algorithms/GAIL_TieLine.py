...
class GAIL():
    ...
    def train_d(self, 
        ...
        for _ in range(self.train_iters):
            ...
            batch_exp_prob = self.d_network(exp_obs[exp_sample_indices], batch_exp_action)
            batch_agent_prob = self.d_network(agent_obs[agent_sample_indices], batch_agent_action)
...
class TrainManager():
    def __init__(
        ...
        self.gail = GAIL(self.action_dim, d_network, d_optimizer, self.discriminator_train_iters,self.discriminator_batch_size)
    ...
    def train(self) -> None:
        ...
        with tqdm(total=self.expert_episode_used, desc='Collecting Expert Data') as pbar:
            while sampled_idx < self.expert_episode_used:
                ...
                while not done:
                    ...
                    self.buffer.append((obs, action, next_obs, done)) # append data to buffer
                    obs = next_obs
                    step += 1
                    
                pbar.update(1)
        
        self.exp_obs, self.exp_action, self.exp_next_obs, self.exp_done  = self.buffer.sample(batch_size=self.expert_sample_used,shuffle=True)
        ...
        with tqdm(total=self.episode_num, desc='Training') as pbar:
            for e in range(self.episode_num):
                ...
                pbar.set_postfix({"Avg. Success rate": (avg_test_success_rate)})
                pbar.update(1)
...
if __name__ == "__main__":
    ...
    manager = TrainManager(
        ...
        discriminator_batch_size = args.IL_batch_size,
